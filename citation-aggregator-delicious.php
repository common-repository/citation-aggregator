<?php
/*
 * del.icio.us feed reader - A component of the Citation Aggregator plugin for WordPress
...
/**
 * citation_aggregator_delicious class contains all of the code for parsing of a del.icio.us feed
 */

if (!class_exists('citation_aggregator_delicious')) {

	class citation_aggregator_delicious {
	
		// Declare global variables for the class
		var $feed_data = '';
		var $version = '1.0';
		var $feed_type = 'delicious';
		...
		function parse_feed() {
			...
			if(get_option($cit_aggie->admin_opt_name . '_enable_delicious') != 'yes') {
				return FALSE;
			}
			
			// if we get this far we should be parsing the feed
			$feed_url = get_option($cit_aggie->admin_opt_name . '_delicious_tag');
			if($feed_url == FALSE || $feed_url == '') {
				return FALSE;
			}
			
			// Build the URL of the feed
			$feed_url = 'http://feeds.delicious.com/rss/tag/' . $feed_url;
			
			// check to ensure the SimplePie Class is avaialable
			$cit_aggie->check_simple_pie();
			
			// Attempt to parse the feed
			// Suppress any warnings from the library
...
#			return $debug;
			...
			$users = get_option($cit_aggie->admin_opt_name . '_delicious_users');
